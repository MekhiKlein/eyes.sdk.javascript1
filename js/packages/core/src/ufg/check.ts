...
export function makeCheck<TSpec extends SpecType>({
  ...
  return async function check({
    ...
    if (driver) {
      ...
      snapshotResults = !utils.types.isArray(target)
        ? Array(uniqueRenderers.length).fill({snapshot: target})
        : target.map(snapshot => ({snapshot}))
      snapshotUrl =
        snapshotResults.length > 0 && utils.types.has(snapshotResults[0].snapshot, 'url')
          ? snapshotResults[0].snapshot.url
          : undefined
    }
