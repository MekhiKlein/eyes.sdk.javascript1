...
export type RenderEnvironment = (
  | DesktopBrowserEnvironment
  | ChromeEmulationDeviceEnvironment
  | IOSDeviceEnvironment
  | AndroidDeviceEnvironment
) & {
  environmentId?: string
  properties?: BaseCore.CustomProperty[]
}
export type ExactEnvironment = BaseCore.Environment & {environmentId?: string; requested?: RenderEnvironment}
...
export type CheckSettings<TSpec extends SpecType> = BaseCore.CheckSettings<Region | ElementReference<TSpec>> &
  ScreenshotSettings<TSpec> & {environments?: Environment[]}

export type CloseSettings = BaseCore.CloseSettings & {environments?: Environment[]}

export type AbortSettings = BaseCore.AbortSettings & {environments?: Environment[]}
