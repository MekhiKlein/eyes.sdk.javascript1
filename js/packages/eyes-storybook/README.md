...
| `exitcode`                | true                       | If tests failed or has visual differences close with non-zero exit code (also available as command-line argument). If exiting with exitcode `1` is requiered only for errors and not visual differences, set the value to `nodiffs` |
